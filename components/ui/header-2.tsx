...
import { siteConfig, type NavVariant } from '@/config/site';

function navVariantClasses(variant: NavVariant = 'link'): string {
  switch (variant) {
    case 'button':
      return 'px-3 py-1.5 rounded-md bg-foreground text-background text-xs font-semibold hover:bg-foreground/90 transition-colors';
    case 'outline':
      return 'px-3 py-1 text-xs font-semibold text-foreground border border-border rounded hover:bg-accent hover:text-accent-foreground transition-colors';
    case 'ghost':
      return 'px-2 py-1 text-xs text-foreground/80 hover:text-foreground hover:bg-accent/40 rounded transition-colors';
    case 'secondary':
      return 'px-3 py-1 text-xs font-semibold bg-secondary text-secondary-foreground rounded hover:bg-secondary/90 transition-colors';
    case 'link':
    default:
      return 'text-xs text-foreground/80 hover:text-foreground transition-colors';
  }
}
				...
				<div className="hidden items-center gap-2 md:flex">
					{siteConfig.nav.primary.map((item) => (
						<Link
							...
							href={item.href}
							className={cn(
								navVariantClasses(item.variant),
								'px-3 py-1'
							)}
							{...(item.external
					...
					<div className="flex items-center gap-2">
						<SignedOut>
							<Link
								href={siteConfig.nav.auth.signIn.href}
								className={navVariantClasses(
									siteConfig.nav.auth.signIn.variant
								)}
							>
								{siteConfig.nav.auth.signIn.label}
							</Link>
							<Link
								href={siteConfig.nav.auth.signUp.href}
								className={navVariantClasses(
									siteConfig.nav.auth.signUp.variant
								)}
							>
								{siteConfig.nav.auth.signUp.label}
							</Link>
						</SignedOut>
						...
						<SignedIn>
							<Link
								href={siteConfig.nav.auth.dashboard.href}
								className={navVariantClasses(
									siteConfig.nav.auth.dashboard.variant
								)}
							>
								{siteConfig.nav.auth.dashboard.label}
							</Link>
							<SignOutButton>
								<button
									type="button"
									className="px-3 py-1 text-xs font-semibold text-red-500 border border-red-500 rounded hover:bg-red-500 hover:text-white transition-colors"
								>
									Sign out
								</button>
							</SignOutButton>
					...
					<div className="grid gap-y-2">
						{siteConfig.nav.primary.map((item) => (
							<Link
								...
								className={cn(
									navVariantClasses(item.variant),
									'w-full px-3 py-2 text-xs font-semibold text-left border border-border rounded hover:bg-accent hover:text-accent-foreground transition-colors',
								)}
								{...(item.external
									? { target: '_blank', rel: 'noreferrer' }
									: {})}
						...
						<SignedIn>
							<div className="flex flex-col items-center gap-2 w-full">
								...
								<SignOutButton>
									<button
										type="button"
										className="w-full px-3 py-2 text-xs font-semibold text-red-500 border border-red-500 rounded hover:bg-red-500 hover:text-white transition-colors"
										onClick={() => setOpen(false)}
									>
										Sign out
									</button>
